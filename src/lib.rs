...
`O(m * n)` временную сложность, где `m` соответствует регулярного выражения и `n` 
...
* [В разделе «Функции дополнения»](#Описание_дополнения) описываются возможности дополнения, которые можно включить или отключить для этого дополнения.
...
(Точнее, в этом дополнении это означает «соответствует любой кодировке UTF-8 любого одиночного знака Unicode, отличного от `\n`).
...
несколькими способами. Все ниже указанные примеры равноценны:
...
* Когда режим Unicode включен (по умолчанию), `.` будет соответствовать любому одиночному знаку Unicode, 
...
условие отключено, `.` будет соответствовать любому байту вместо любого одиночного знака Unicode.
