...
`O(m * n)` временную сложность, где `m` соразмерна размеру регулярного выражения и `n` 
соразмерна размеру искомой строки.

[regular expression]: https://en.wikipedia.org/wiki/Regular_expression

IЕсли вам нужна только руководство API, то переходите к  [`Regex`] виду. В противном случае, вот краткий пример, 
...
[`RegexBuilder`] позволяет задать настройку, отличную от настройки по умолчанию. (Например, соответствие без учета регистра, подробный режим и другие.)
...
exists. [`Regex::find_iter`] возвращает повторитель по всем таким совпадениям.
...
В противном случае это руководство по ящику верхнего уровня представлено следующим образом:
...
* [Правила написания](#Правила написания) принятые для регулярных выражений, поддерживаемые этим дополнением в Ржавчине.
* [В разделе «Ненадежные входные данные»](#Ненадежный ввод) рассматривается, как это дополнение справляется с образцами регулярных 
выражений или стогами сена, которые не являются надежными.
* [В разделе «Функции ящика»](#Описание дополнения) описываются функции груза, которые можно включить или отключить для этого ящика.
* [Другие дополнения](#Другие дополнения) связаны с другими ящиками в `regex` семействе.
...
создает исходная рукопись для поиска по регулярному выражению, а затем запускает программу.
...
* **regex** is это значение в Ржавчине, вид которого `Regex`. Мы используем `re` 
...
в качестве имени переменной для образца.
...
(Точнее, в этом дополнении это означает «соответствует любой кодировке UTF-8 любого одиночного значения Unicode, отличного от `\n``.»)
* Мы можем сопоставить действительное `.` буквально, экранировав его, т. е `\.`.
* Мы используем необработанные [строки в Ржавчине] , чтобы избежать необходимости иметь дело с escape-последовательностями как в правилах писания образца 
регулярных выражений, так и в правилах писания строковых знаков в Ржавчине. Если бы мы не использовали здесь необработанные строки, нам пришлось бы использовать 
`\\.` для сопоставления знака `.` знака. То есть `r"\."` и `"\\."` являются равноценными образцами.
...
Присвоение имени группе может быть полезным, когда в образце есть несколько групп. Это делает рукопись, ссылающийся на эти группы, немного более понятным.

### Пример: проверка определенной разметки даты

В этом примере показано, как проверить, соответствует ли стог сена в целом определенной разметки даты:
...
Вообще говоря, это дополнение мог бы предоставить функцию для ответа только на #3, которая самостоятельно включала бы #1 и #2. 
...
Это дополнение имеет первоклассную поддержку Unicode и **включен по умолчанию** . 
...
Хотя `\w` вряд ли будет замечено, знак, `\w{100}` например, приведет к довольно большому 
...
хорошей маслью придерживаться собраний ASCII. Версия только для ASCII `\w` может быть написана 
несколькими способами. Все следующие равноценны:
...
### Избегайте повторной сборки регулярных выражений, особенно в цикле.

Сборка одного и того же образца в цикле является антиобразцом, 
...
### Совместное использование регулярного выражения в нескольких потоках может привести к конфликтам

Хотя один `regex` может свободно использоваться из нескольких потоков одновременно, есть 
небольшая стоимость согласования, которую необходимо оплатить. Вообще говоря, не следует 
...
В этом разделе рассматривается, какой вид поддержки Unicode имеет эта библиотека регулярных выражений. Прежде чем показать несколько примеров, мы суммируем соответствующие особенности:
...
* Верхний уровень [`Regex`] запускает поиск , как будто перебирая каждую из кодовых точек в стоге сена. 
То есть, фундаментальным атомом сопоставления является одна кодовая точка.
* [`bytes::Regex`], напротив, позволяет отключить режим Unicode для части всего вашего образца во всех 
случаях. Когда режим Unicode отключен, поиск выполняется так, *как будто* он перебирает каждый байт в 
стоге сена. То есть, фундаментальным атомом сопоставления является один байт. (Верхний уровень Regex
также позволяет отключить Unicode и, таким образом, сопоставление, как если бы это был один байт за раз, но 
только тогда, когда это не позволит сопоставить недействительный UTF-8.)
* Когда режим Unicode включен (по умолчанию), `.` будет соответствовать всему одиночному значению Unicode, 
даже если оно закодировано с использованием нескольких байтов. Когда режим Unicode отключен (например, `(?-u:.)`), `.`
будет соответствовать одному байту во всех случаях.
* Собрания знаков `\w`, `\d` и `\s` по умолчанию поддерживают Unicode. Используйте `(?-u:\w)`, `(?-u:\d)` и , `(?-u:\s)` 
чтобы получить их определения только для ASCII.
* Также как, `\b` и `\B` используйте определение Unicode знака «слово». Чтобы получить границы слов 
...
* Поиск без учета регистра поддерживает Unicode и использует простое преобразование регистра.
...
Сами образцы представляются только как последовательность одиночных значений Unicode. 
...
знаков Unicode. Например, чтобы сопоставить любую кодовую точку, которая есть как в Greek сценарии, 
...
если это может привести к сопоставлению с недопустимым UTF-8. Например, когда `u` 
условие отключено, `.` будет соответствовать любому байту вместо любого одиночного значения Unicode.
...
Любое именованное собрание знаков может появляться внутри заключенного в скобки `[...]` собрания знаков. Например,
 `[\p{Greek}[:digit:]]` соответствует любой цифре ASCII или любой кодовой точке в
...
кодовую точку, обязательно никогда не будет возвращено. Однако такие соответствия 
...
Имена групп захвата должны быть любой последовательностью буквенно-цифровых кодовых точек Unicode, в дополнение к
 `.`, `_`, `[` и `]`. Имена должны начинаться либо с `_` ибо с буквенной кодовой точки. 
 Буквенные кодовые точки соответствуют свойству `Alphabetic`
Unicode, тогда как числовые кодовые точки соответствуют объединению
...
<pre class="rust">
i     без учета регистра: буквы соответствуют как верхнему, так и нижнему регистру
...
Условия можно переключать в образце. Вот пример, который соответствует без учета регистра для первой части, но с учетом регистра для второй части:
```rust
use regex::Regex;
...
За исключением четко документированных случаев, большинство API в этом ящике предназначены для того, 
...
возможно возникновение сбоя. Если вам нужна надежное заверение против сбоев, то вам следует обернуть 
...
Например, рассмотрим образец  `.*[^A-Z]|[A-Z]` и стог сена `AAAAA`. TПервый поиск будет сканировать до конца в поисках совпадений, 
 `.*[^A-Z]` хотя конечный автомат (как в этом ящике) знает, что `[A-Z]` уже совпал с первым знаком стога сена. Это связано 
...
возможных способов смягчить это — спуститься на нижний уровень  `regex-automata` ящика и использовать его
...
не удастся, поскольку нечувствительность к регистру Unicode включена по умолчанию. 
Вместо этого вызывающие должны использовать `(?i-u)a` для отключения сворачивания регистра Unicode. 
...
* **std** -
  ...
  [`regex-automata`](https://docs.rs/regex-automata) ящику.
* **logging** -
...
* **perf-onepass** -
 ...
 собой самый быстрый движок в этом ящике для работы с группами захвата.
* **perf-backtrack** -
...
* **unicode-age** -
 Предоставьте данные для
  ...
  для ссылки на все кодовые точки, впервые представленные в Unicode 6.0
* **unicode-bool** -
...
* **unicode-case** -
  предоставление данных для сопоставления без учета регистра с использованием
...
компоненты этого ящика могут использоваться независимо.
...
стандартной библиотеки определенной среды. Действительно, для других также как оптимизированных
 движков регулярных выражений большая часть или вся рукопись в зависимостях этого ящика обычно были бы 
 просто неразделимыми или связанными частями самого ящика. Но поскольку Ржавчина и его инструментальная 
 экосистема делают использование зависимостей таким простым, имело смысл потратить некоторые усилия 
 на разделение частей этого ящика и сделать их независимо полезными.

Здесь мы лишь кратко описываем каждый ящик.
...
Другими словами, он предоставляет быстрые процедуры `memchr` и `memmem`. Они используются этим ящиком в знаковых оптимизациях.
...
поиска относительно невелико `regex` Ящик также использует это для знаковых оптимизаций.
*/
