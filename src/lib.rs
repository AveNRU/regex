...
* [`Regex::find_iter`] возвращает количество совпадений условия выражения.
...
* [`Regex::captures_iter`] возвращает количество совпадений условия выражения.
...
**Стог сена (haystack)**  - это строка, в которой регулярное выражение ищет совпадения. Мы используем `hay` 
...
Что касается скорости поиска, Unicode, как правило, обрабатывается довольно хорошо, даже при
 ...
 Поэтому, если вам не нужны определения границы слова, поддерживающие Unicode, вы можете рассмотреть 
 возможность использования `(?-u:\b)` вместо `\b`, где первый определяет границы слова только в среде ASCII.
...
в [Unicode Technical Standard #18][UTS18] . Полные сведения о том, что поддерживается, представлены
...
знаков Unicode. Например, чтобы сопоставить любой знак, который есть как в Greek сценарии, 
так и в Letter общих разделах:
...
Способ [`bytes::Regex`], который можно использовать для поиска `&[u8]` в стогах сена. 
По умолчанию стога сена обрабатываются как UTF-8, как и в случае с 
основным `Regex` способом. Однако это поведение можно отключить, отключив `u` условие, даже 
если это может привести к сопоставлению с недопустимым исполнением UTF-8. Например, когда `u` 
условие отключено, значение `.` будет соответствовать любому одному байту, что не соответствует
большинству одиночных знаков Unicode, который представлены как несколько байт (2-4).

Отключение `u` условия также возможно с принятым видом данных - срезом строки `&str` на основе - `regex` , но это 
разрешено только там, где поддерживается неизменное исполнение UTF-8. Например, `(?-u:\w)` является 
поиском любого знака только для ASCII `\w` и является допустимым в `&str` - `Regex`, но `(?-u:\W)` 
...
это дополнение предоставляет рычаги для отключения сборки этих таблиц данных, что может быть 
...
\d            Цифра (\p{Nd})
\D            Не цифра
\pX           Собрание знаков Unicode, определяемое однобуквенным именем
\p{Greek}     Собрание знаков Unicode (общий раздел или сценарий)
\PX           Отрицательное собрание знаков Unicode, определяемое однобуквенным именем
...
[a-z]         Собрание знаков, соответствующее любому знаку в ряде a-z.
...
Любое именованное собрание знаков может появляться внутри заключенного в скобки `[...]` собрания знаков. Например,
 `[\p{Greek}[:digit:]]` соответствует любой цифре ASCII или любому знаку в
...
Имена групп захвата должны быть любой последовательностью буквенно-цифровых знаков Unicode, в дополнение к
 `.`, `_`, `[` и `]`. Имена должны начинаться либо с `_` ибо со знака буквы. 
...
Обратите внимание, что сюда включены все возможные исходы, даже те, которые представлены в другом месте.
...
Не поймите меня неправильно. Это дополнение тщательно проверен, не только с помощью раздельных 
...
* **std** -
   при включении это приведет `regex` к использованию встроенной библиотеки. 
...
* **perf-cache** -
  ...
  но теперь это не исход. Быстрый внутренний кэш теперь используется безусловно без дополнительных зависимостей. Это может измениться в будущем.
...
* **unicode-bool** -
   предоставляет данные для многочисленных разумных свойств Unicode. Полный список здесь не включен, 
...
* **unicode-perl** -
  Предоставьте данные для поддержки собраний знаков Perl, поддерживающих Unicode, соответствующих
   `\w`, `\s` and `\d`. Это также необходимо для использования определений границ слов, 
...
встроенной библиотеки определенной среды. Действительно, для других также как оптимизированных
...
* [`regex-syntax`](https://docs.rs/regex-syntax)предоставляет обработчик регулярных выражений через
 ...
 выражений, не беспокоясь о написании обработчика.
